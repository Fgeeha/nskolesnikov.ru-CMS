...
ARTICLE = {
    ...
    "body": (
        "Когда RAG собирается в закрытом контуре, привычные компромиссы меняются. "
        "Нельзя дёрнуть внешний API, нельзя опереться на чужие эмбеддинги — всё "
        "работает на своём железе, и любое лишнее звено сразу видно в latency.\n\n"
        "Первым ломается не генерация, а ретривер. Модель отвечает связно и уверенно "
        "даже на нерелевантном контексте, поэтому качество поиска приходится измерять "
        "отдельно, до подключения LLM.\n\n"
        "Второй частый источник проблем — нарезка документов. Фиксированный размер "
        "чанка разрывает таблицы и списки, а именно они чаще всего содержат ответ. "
        "Разметка по структуре документа даёт больше, чем подбор длины окна.\n\n"
        "Третье — наблюдаемость. Без логов запросов и метрик попадания в контекст "
        "система превращается в чёрный ящик, который невозможно улучшать осознанно."
    ),
    "reading_time": 4,
